...
_spec = importlib.util.spec_from_file_location('rect_search_main', os.path.join(_parent, 'rect_search_generator', 'main.py'))
_mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_mod)
RectSearchGenerator = _mod.RectSearchGenerator
...
class TestRectSearchGenerator(unittest.TestCase):
    ...
    def test_zigzag_behavior_alternates_direction(self):
        """Confirm it always uses zigzag behavior (alternating direction)."""
        gen = RectSearchGenerator()
        rect = {"center": {"x": 0, "y": 0}, "width": 10, "height": 10, "yaw": 0}
        ctx = self._make_context(rect=rect, num_lines=2)
        result = gen.generate(ctx)
        
        # Line 0: BL -> BR (roughly)
        # Line 1: BR -> BL (reversed)
        ...
        self.assertTrue(x1_s > x1_e) # Reversed!
