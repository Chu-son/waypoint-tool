...
pub fn run_wasm_plugin(wasm_path: &str, context_json: &str) -> Result<serde_json::Value, String> {
    ...
    let mut child = Command::new("wasmtime")
        ...
        .map_err(|e| {
            format!(
                "Failed to spawn wasmtime CLI. Is wasmtime installed in PATH?\nError: {}",
                e
            )
        })?;
    ...
    let result: serde_json::Value = serde_json::from_str(&stdout_str).map_err(|e| {
        format!(
            "Failed to parse wasm plugin output as JSON: {}\nOutput was:\n{}",
            e, stdout_str
        )
    })?;
