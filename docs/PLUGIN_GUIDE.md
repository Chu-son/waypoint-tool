...
2. 「Add Folder」をクリックし、プラグインのディレクトリを選択します。
   - **単一プラグインのインポート**: `manifest.json` が直下に存在するプラグインフォルダを指定します。
   - **複数プラグインの一括インポート**: 複数のプラグインサブディレクトリが含まれる親フォルダを指定すると、配下のプラグイン（深さ3階層まで）を一括検出してまとめてインポートします（`.git`, `node_modules`, `.venv`, `__pycache__` 等は自動スキップされます）。
   - **複数ディレクトリの選択**: ダイアログで複数フォルダを同時に選択してインポートすることも可能です。
