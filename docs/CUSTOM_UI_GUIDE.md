...
### 3.1 テーマ設定とプリセット (`theme`)

UI全体のカラーテーマ（背景色、パネル色、ボタン色、テキスト色、枠線色など）を容易にカスタマイズできます。

| プリセット名 | 表示名 / 雰囲気 | メイン色 (Primary) | ホバー色 (Hover) | 基底背景 (surfaceBase) | パネル背景 (surfacePanel) | 枠線 (borderBase) |
|---|---|---|---|---|---|---|
| **`default`** / **`dark`** | Linear Indigo (標準ダーク) | `#5e6ad2` | `#6f7be8` | `#090a0c` | `#121316` | `rgba(255, 255, 255, 0.08)` |
| **`roomba`** / **`emerald`** | Emerald (クリーン・エコ) | `#10b981` | `#34d399` | `#080c0a` | `#101512` | `rgba(255, 255, 255, 0.08)` |
| **`ocean`** | Ocean (ディープブルー) | `#0ea5e9` | `#38bdf8` | `#080a0f` | `#0f131a` | `rgba(255, 255, 255, 0.08)` |
| **`amber`** | Amber (ウォームゴールド) | `#f59e0b` | `#fbbf24` | `#0c0a09` | `#161311` | `rgba(255, 255, 255, 0.08)` |
| **`purple`** | Purple (エレクトリックバイオレット) | `#8b5cf6` | `#a78bfa` | `#09080e` | `#121018` | `rgba(255, 255, 255, 0.08)` |
| **`midnight`** | Midnight (OLEDピュアブラック) | `#06b6d4` | `#22d3ee` | `#000000` | `#0c0c0d` | `rgba(255, 255, 255, 0.08)` |
| **`light`** | Linear Light (標準ライト) | `#5e6ad2` | `#4b55c0` | `#f7f8f9` | `#ffffff` | `#e2e4e8` |

### Linear Style テーマ策定の 5 つの黄金律
カスタムテーマを定義する場合も、ツール全体のデザイン統一感を維持するため以下の 5 原則を遵守してください：
1. **基底サーフェス彩度制限律 (Saturation < 5%)**: `surfaceBase` と `surfacePanel` は彩度 5% 未満のディープチャコール（明度 3〜8%）に抑えること。画面全体が原色に染まる「ネオンテーマ」は禁止。
2. **文字ニュートラル不変律 (Neutral Text Invariance)**: `textBase`（`#f7f8f8`）および `textMuted`（`#8a8f98`）はニュートラルを保ち、テキストへの着色を行わないこと。
3. **1px ヘアライン半透明枠線律**: `borderBase` は原則透過アルファ `rgba(255, 255, 255, 0.08)` を使用すること。
4. **アクセントコントラスト律 (WCAG AA 準拠)**: `primaryBase` は `surfaceBase` に対して十分なコントラスト比（4.5:1 以上）を確保すること。
5. **キャンバス・PixiJS 整合性**: キャンバス描画領域の背景色は、ROSマップ（白領域=254）との高コントラストを担保するため常にダークチャコール（`CANVAS_SURFACE_BASE` = `0x090a0c`）を維持します（CAD・RViz方式。`customUiConfig` で明示的に `surfaceBase` が指定された場合を除く）。

> [!NOTE]
> **Linear Light Theme 仕様**:
> 単にダークモードの色を反転させただけの純白ではなく、長時間作業でも目が疲れない微細なオフホワイト（`#f7f8f9`）、パネル用のピュアホワイト（`#ffffff`）、深いチャコールグレー（`#17171a`）、そしてダークモードと共通のブランドインディゴ（`#5e6ad2`、ホバー時 `#4b55c0`）を軸とした洗練されたライトモード仕様です。また、一般設定モーダル（General）のテーマセレクターからもワンクリックでダーク／ライトを切り替え、ライトモードのまま任意のアクセントテーマ（Indigo, Emerald, Ocean, Amber, Purple, Midnight）を自由に選択可能です。さらに、ROS 2D占有グリッドマップ（空き領域が白 254）とのコントラストを保護するため、ビューポート描画領域は常に視認性の高いダークチャコール背景（Option A - CAD / RViz 方式）を維持します。
...
### 3.2 ステップ定義と複数アクションボタン (`actionButtons`)
