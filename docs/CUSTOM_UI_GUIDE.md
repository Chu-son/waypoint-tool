...
| プリセット名 | 雰囲気 / 用途 | メインカラー | 背景 / パネル |
|---|---|---|---|
| **`default`** / **`dark`** | スタンダードなダークテーマ (Linear Dark) | Linear インディゴ (`#5e6ad2`) | チャコールブラック (`#090a0c`) / パネル (`#121316`) |
| **`roomba` / `emerald`** | 清掃・フレッシュ・エコ | エメラルドグリーン (`#10b981`) | ナイトグリーン (`#091e17`) |
| **`ocean`** | テクノロジー・深海ブルー | スカイブルー (`#0284c7`) | ミッドナイトネイビー (`#030712`) |
| **`amber`** | 工事・現場・建設 | アンバーゴールド (`#f59e0b`) | チャコールグレー (`#18181b`) |
| **`purple`** | 先進的・AI | ネオンパープル (`#8b5cf6`) | ディープバイオレット (`#0f0728`) |
| **`midnight`** | コントラスト重視 | クールシアン (`#06b6d4`) | ピュアブラック (`#000000`) |
| **`light`** | スタイリッシュなライトテーマ (Linear Light) | Linear インディゴ (`#5e6ad2`) | オフホワイト (`#f7f8f9`) / パネル白 (`#ffffff`) |

> [!NOTE]
> **Linear Light Theme 仕様**:
> 単にダークモードの色を反転させただけの純白ではなく、長時間作業でも目が疲れない微細なオフホワイト（`#f7f8f9`）、パネル用のピュアホワイト（`#ffffff`）、深いチャコールグレー（`#17171a`）、そしてダークモードと共通のブランドインディゴ（`#5e6ad2`、ホバー時 `#4b55c0`）を軸とした洗練されたライトモード仕様です。また、一般設定モーダル（General）のテーマセレクターからもワンクリックでダーク／ライトを切り替え可能です。
