...
本プロジェクトでは、`src/components/` 以下のコンポーネントを**役割ベース**で分割することを基本とします。

### 2.1 基本的なディレクトリ分類

新規コンポーネントを作成する際は、原則として以下の既存分類のいずれかに配置してください。

- **`ui/`**: 汎用・再利用可能なUI部品
  - ボタン、入力欄、メニュー、モーダルなど、特定のドメインロジック（Waypointのデータ構造など）に強く依存しない純粋な表示要素。
- **`canvas/`**: 描画関連
  - `MapCanvas` など、マップ描画やWebGL/2Dコンテキストを扱うコアな描画コンポーネント。
- **`settings/`**: 設定関連
  - アプリケーション全体の設定や、設定画面に関するコンポーネント。
- **`common/`**: 特定機能に依存しない共通機能
  - UIとして画面には現れないがアプリケーション全体で必要な機能（例: `ShortcutManager`）など。
