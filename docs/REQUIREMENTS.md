...
- **Python / Rust (WASM) プラグインの実行**: 外部スクリプト（Python または `.wasm`）を「Generator」として呼び出し、任意の自動生成アルゴリズムを組み込めること。
  - プラグインは `manifest.json` でメタデータ（名前・説明・入力・プロパティ・UI型）を宣言し、フロントエンドが動的に入力フォームを生成すること。
  - **プロパティのインタラクションヒント (`interaction_hints`)**: 各プロパティに対し、キャンバス上の特定の入力データに視覚的な装飾（例: スイープの開始コーナーや方向インジケータ）を描画するためのヒント情報を定義できること。これにより、コアコードを修正することなくプラグイン固有の補助図形を表示可能にすること。
  - 出力は `stdout` を経由した JSON 配列形式の Waypoint データ（`transform` 形式: `{"transform": {"x","y","qx","qy","qz","qw"}, "options": {...}}`）。
- **インタラクション入力（MapCanvas 上の操作入力）**: `manifest.json` の `inputs` 配列で宣言した入力タイプに応じて、ユーザーがキャンバス上で直接データを定義できること。対応するインタラクション入力タイプ:
  - `point`: クリックで座標を配置し、ドラッグでヨー角を設定。
  - `rectangle`: ドラッグで矩形を描画し、コーナーハンドルでリサイズ、回転ハンドルで回転。
  - `polygon`: 多角形の頂点を順次クリックで定義（将来対応予定）。
  - `path`: 連続したポイント列をクリックで定義（将来対応予定）。
  - `node_select`: 既存の Waypoint を選択して入力として使用（将来対応予定）。
- **複数インタラクション入力のサポート**: 一つのプラグインが `inputs` 配列に複数の入力を宣言でき、フロントエンドがステップ UI（ステップインジケータ、入力完了時の自動遷移）を表示して順次入力を受け付けること。
...
- **プラグイン雛形生成（Scaffolding）**: Settings > Plugins からプラグイン名と保存先ディレクトリを指定して、新規プラグインの雛形（`manifest.json`、`main.py`、`wpt_plugin.py` SDK コピー）を自動生成できること。生成後は自動的にカスタムプラグインとして登録されること。
- **SDK バージョン管理**: 各プラグインディレクトリ内の `wpt_plugin.py`（SDK）のバージョンを検出し、組み込み SDK のバージョンと比較すること。
  - プラグイン一覧で SDK の状態（✅ 最新 / ⚠️ 古い / ❌ 未検出）をバッジで表示すること。
  - 古い SDK を検出した場合、ワンクリックで最新版に更新できる「Update SDK」ボタンを提供すること。
...
### 11. ジェネレーターノード編集時のリアルタイムキャンバスプレビューとヒント表示 (Generator Edit Preview & Generic Hints)
- Properties Panel でジェネレーターノードの入力パラメータ（例: `start_point` の X・Y・Yaw、または `interaction_hints` が指定されたプロパティ値）を変更した際、**Re-Generate を実行する前でも**、キャンバス上のプレビューや補助インジケータ（ピンク矢印、開始コーナー矢印など）がリアルタイムに更新されて表示されること。
- このプレビューおよび補助表示は、Generator ノードが選択状態にある限り表示し続けること。
- コアの描画ロジックは特定のプラグイン名やプロパティ名に依存せず、マニフェストで定義されたヒント情報に基づき汎用的に処理されること（疎結合の維持）。
