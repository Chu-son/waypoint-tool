...
function DropdownMenu({ 
  label, options, isOpen, onClick, onClose, onMouseEnter 
}: { 
  label: string; options: MenuOption[]; isOpen: boolean; onClick: () => void; onClose: () => void; onMouseEnter: () => void 
}) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button 
        ...
        className={`px-3 py-1.5 rounded text-[13px] font-medium transition-colors ${isOpen ? 'bg-slate-700/80 text-white shadow-inner' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}
      >
        {label}
      </button>
      ...
      {isOpen && (
        <div className="absolute left-0 top-full mt-1 w-56 bg-slate-800 border border-slate-700 shadow-2xl rounded py-1 z-50">
          {options.map((opt, i) => opt.divider ? (
             ...
             >
               <span>{opt.label}</span>
               {opt.shortcut && <span className="text-[11px] text-slate-500 font-mono tracking-tighter truncate ml-2 group-hover:text-primary-200">{opt.shortcut}</span>}
             </button>
          ))}
        </div>
      )}
...
export function TopMenu() {
  ...
  const editOptions: MenuOption[] = [
    { label: 'Delete Selected', action: () => { if(selectedNodeIds.length > 0) removeNodes(selectedNodeIds); }, shortcut: 'Del / Backspace' }
  ];
    ...
    <div className="h-9 bg-slate-950 border-b border-slate-800 flex items-center px-4 shrink-0 text-slate-300 z-50 relative select-none shadow-sm">
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-2 text-slate-100 font-bold tracking-wide pointer-events-none">
           <MousePointer2 size={16} className="text-primary rotate-45 transform fill-primary" />
