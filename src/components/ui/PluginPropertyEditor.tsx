    ...
    <div className={`space-y-1 ${className}`}>
      ...
      {property.type === 'boolean' ? (
        ...
        <input 
          ...
          value={value ?? ''}
          onChange={e => {
            const val = property.type === 'float' ? parseFloat(e.target.value) : parseInt(e.target.value, 10);
            onChange(isNaN(val) ? '' : val);
          }}
          className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-primary placeholder:text-slate-600 transition-colors"
        ...
        <select
          ...
          className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1.5 text-xs text-white focus:outline-none focus:border-primary cursor-pointer transition-colors"
        >
      ...
      {property.description && (
        <p className="text-[10px] text-slate-500 leading-tight mt-1">{property.description}</p>
      )}
    </div>
  );
};
