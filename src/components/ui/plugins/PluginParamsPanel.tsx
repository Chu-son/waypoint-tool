      ...
      await runWithLoading(
        ...
        async () => {
          ...
          const result = await useAppStore.getState().executeGeneratorPlugin({
            ...
            placement:
              idsToConsume.length > 0 ? { type: 'replace_ids', ids: idsToConsume } : { type: 'use_insertion_target' },
          });
      ...
      <div className="space-y-4 flex-1">
        ...
        {inputs.length > 1 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-[11px] font-semibold text-text-muted uppercase tracking-wider">Input Steps</Label>
                ...
                return (
                  <Button
                    ...
                    className={cn(
                      ...
                      !isActive &&
                        !hasData &&
                        'bg-surface-panel/50 border-border-base/50 text-text-muted hover:border-border-base',
                    )}
                    title={inp.label || key}
                  >
                    <div className="flex items-center justify-center gap-1">
                      <span
                        className={cn(
                          ...
                          isActive
                            ? 'bg-primary-base text-text-inverse'
                            : hasData
        ...
        {properties.length > 0 &&
          properties.map((prop, idx) => {
            ...
            if (!key) return null;
            return (
              <div key={`prop-container-${idx}`} className="mt-3 pt-3 border-t border-border-base/50">
        ...
        {needsSelection && (
          <AlertBox title="Requires Waypoint Selection" className="mt-4">
            You currently have <strong className="text-primary-base">{selectedNodeIds.length}</strong> points selected.
          </AlertBox>
        )}

        {errorInfo && (
          <AlertBox title="Execution Error" variant="danger" className="mt-4">
            <div className="font-mono whitespace-pre-wrap break-all overflow-auto max-h-32">{errorInfo}</div>
          </AlertBox>
        )}
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-border-base/50">
        ...
        >
          {isExecuting ? (
            ...
            <Play
              size={14}
              className={cn('mr-2', !(needsSelection && selectedNodeIds.length === 0) && 'fill-current')}
            />
          )}
