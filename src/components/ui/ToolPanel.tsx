...
import { MousePointer2, Download, Settings, Plus, MoreHorizontal, Puzzle } from 'lucide-react';
...
export function ToolPanel() {
  ...
  const enabledPluginsList = pluginSettings.filter(s => s.enabled).sort((a,b) => a.order - b.order).map(s => plugins[s.id]).filter(Boolean);
  ...
  return (
    <div ref={panelRef} className="bg-slate-800 border-r border-slate-700 flex flex-col items-center py-4 px-2 gap-4 z-10 shadow-md transition-all duration-300 relative" style={{ minWidth: '4rem', width: 'auto' }}>
      ...
      {tools.map(tool => {
        ...
        return (
          <button
            key={tool.id}
            onClick={() => { setActiveTool(tool.id); setActivePlugin(null); }}
            title={tool.label}
            className={`
              w-10 h-10 rounded-xl transition-all flex items-center justify-center group flex-shrink-0
            ...
            `}
          >
            <Icon size={20} className={isActive && !activePluginId ? '' : 'group-hover:scale-110 transition-transform'} />
          </button>
        );
      })}
                ...
                {overflowPlugins.map(plugin => {
                  ...
                  return (
                    <button
                      ...
                      `}
                    >
                      <Puzzle size={14} className={isActive ? 'text-primary-300' : 'text-slate-500'} />
