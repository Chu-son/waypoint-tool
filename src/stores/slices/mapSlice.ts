...
export const createMapSlice: StateCreator<AppState, [], [], MapSlice> = (set, get) => ({
  ...
  reorderCustomLayers: (fromIndex: number, toIndex: number) =>
    set((state) => {
  ...
  updateMapLayer: (id: string, updates: Partial<ProjectMapLayer>) =>
    set((state) => ({
  ...
  reorderMapLayers: (fromIndex: number, toIndex: number) =>
    set((state) => {
